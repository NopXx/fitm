...
@section('script-app')
    ...
    `;
        ...
        App.NewsFilters = {
            init() {
                ...
                if (window.location.search.includes('debug=true')) {
                    ...
                    // Add event listeners to log on filter change
                    const typeFilter = document.getElementById('newsTypeFilter');
                    const issueFilter = document.getElementById('issueFilter');

                    if (typeFilter) {
                        typeFilter.addEventListener('change', this.debugFilters.bind(this));
                    }

                    if (issueFilter) {
                        issueFilter.addEventListener('change', this.debugFilters.bind(this));
                    }
                }
            },
            ...
            setupFilter(filterElement, resetElement, gridSelector, cardSelector, dataAttribute, noResultsTitle,
                ...
                const filterCards = () => {
                    ...
                    // Trigger search re-apply if search is active
        ...
        `);
            ...
            logFilterData() {
                // Helper function to log card data
                const logCards = (selector, attributeName, containerName) => {
                    console.log(`=== ${containerName} Cards Data ===`);
                    document.querySelectorAll(selector).forEach((card, index) => {
                        console.log(`Card ${index}:`, {
                            [attributeName]: card.getAttribute(`data-${attributeName}`),
                ...
                logCards('#regularNewsGrid .news-card', 'type', 'Regular News');

                // Log FITM news cards data
                logCards('#fitmNewsGrid .fitm-news-card', 'issue', 'FITM News');

                // Log filter options
                const logFilterOptions = (filterId, name) => {
                    const filter = document.getElementById(filterId);
                    if (filter) {
                        console.log(`${name} Filter Options:`, Array.from(filter.options).map(opt => ({
                            value: opt.value,
                            text: opt.text
                        })));
                    }
                ...
                logFilterOptions('newsTypeFilter', 'Type');
                logFilterOptions('issueFilter', 'Issue');
            },
            ...
            debugFilters: function() {
                ...
                document.querySelectorAll('.news-card').forEach((card, i) => {
                    console.log(`Regular Card ${i}:`, {
                        type: card.getAttribute('data-type'),
                        visible: card.style.display !== 'none'
                    });
                });

                console.log("=== FITM Cards ===");
                document.querySelectorAll('.fitm-news-card').forEach((card, i) => {
                    console.log(`FITM Card ${i}:`, {
